...
const STORY_LINES: StoryLine[] = [
  ...
  {
    id: "story-wiki-config",
    title: "配置知识库",
    scenario: "「知识库管理员扔来几个文档链接，让 Harness Agent 更新知识库并验证效果」",
    category: "harness",
    steps: [
      {
        id: "s8-think1",
        type: "think",
        label: "理解任务",
        detail: "管理员给了几个文档链接，需要：1) 读取链接内容 2) 判断每篇是更新已有 WIKI 还是新增 3) 完成知识库更新。先把内容读进来。",
        nodes: [],
      },
      {
        id: "s8-exec1",
        type: "execute",
        label: "Files.write(fetch_docs.py, execute_afterwards=true)",
        detail: "写脚本批量读取管理员给的文档链接内容，将每篇文档的标题、摘要和正文拉取到本地。",
        nodes: ["filesystem", "files", "sandbox"],
        flows: [["filesystem", "files"], ["files", "sandbox"]],
      },
      {
        id: "s8-exec2",
        type: "execute",
        label: "Asset Explorer.search(现有 WIKI 目录)",
        detail: "搜索现有知识库中的 WIKI 资产，获取已有文档列表，用于判断新文档是「更新已有」还是「新增」。",
        nodes: ["asset-explorer", "wiki"],
        flows: [["asset", "asset-explorer"], ["asset-explorer", "wiki"]],
      },
      {
        id: "s8-think2",
        type: "think",
        label: "逐篇判断处理方式",
        detail: "对比新文档与已有 WIKI：内容高度重叠的视为「更新」，新主题的视为「新增」。整理出处理清单。",
        nodes: [],
      },
      {
        id: "s8-exec3",
        type: "execute",
        label: "Asset Editor：更新/新增 WIKI 文档",
        detail: "通过 Asset Editor 逐篇处理：对「更新」类文档合并新旧内容并覆盖原文件；对「新增」类文档直接写入新的 WIKI 资产。",
        nodes: ["asset-editor", "wiki"],
        flows: [["asset-editor", "wiki"]],
      },
      {
        id: "s8-exec4",
        type: "execute",
        label: "调用 Harnessed Agent 验证效果",
        detail: "知识库更新完毕，Harness Agent 直接调用 Harnessed Agent，针对新增/更新的内容提几个测试问题，观察返回结果是否引用了正确的知识库内容。",
        nodes: ["questions", "asset-executor", "asset-explorer", "wiki"],
        flows: [["questions", "asset-executor"], ["asset-executor", "asset-explorer"], ["asset-explorer", "wiki"]],
      },
      {
        id: "s8-think3",
        type: "think",
        label: "评估答案质量",
        detail: "逐条核查 Harnessed Agent 的回答：引用是否准确？有没有答非所问的情况？管理员对不靠谱的答案额外提供了正确答案作为参照。",
        nodes: [],
      },
      {
        id: "s8-exec5",
        type: "execute",
        label: "Asset Editor：二次修订 WIKI",
        detail: "对照正确答案，分析知识库中哪些表述不够精准或有歧义，通过 Asset Editor 再次修订相关 WIKI 文档，使内容更清晰、可检索性更好。",
        nodes: ["asset-editor", "wiki"],
        flows: [["asset-editor", "wiki"]],
      },
      {
        id: "s8-exec6",
        type: "execute",
        label: "Experiments.record + Version 发布上线",
        detail: "管理员确认效果满意，将本次知识库更新记录到 Experiments，生成新 Version 并标记为「已发布」，正式上线。",
        nodes: ["experiments", "version"],
        flows: [["experiments", "version"]],
      },
    ],
